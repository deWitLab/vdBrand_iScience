...
normalise_counts <- function(counts, bins) {
  ...
  # fit <- glm(
  #   count ~ poly(mappa, 3) + poly(gc, 3) + poly(dig, 3),
  #   family = "poisson", data = tmp
  # )
...
df <- data.frame(
  ...
  count = normed[subbins$resolution == 200000, 1]
)
...
```{r}
ggplot(cnts, aes(P06_PE_rep1 + 1, P08_PE + 1)) +
  geom_point(size = 0) +
  scale_x_continuous(
    trans = "log10",
    labels = scales::label_log(),
  ...
  annotate(
    "text", label = paste0("R = ", scales::number(cor["P06_PE_rep1", "P08_PE"], accuracy = 0.001)),
    x = 0, y = Inf, hjust = -0.5, vjust = 2
  )
```

...
```{r load_data}
exps <- load_experiments(resolution = 200e3, sample_subset = c("P01_PB", "P01_LM"))
...
hb <- exps$P01_PB$MAT[join][!is.na(V3) & V3 > 0]
hb[, c("V1", "V2") := list(match(V1, bins$V4), match(V2, bins$V4))]
pb <- exps$P01_LM$MAT[join][!is.na(V3) & V3 > 0]
...
p <- ggplot(data, aes(row, col, fill = value)) + 
  ...
  annotate(
    "label", x = c(-Inf, Inf), y = c(Inf, -Inf), label = c("P01 PB", "P01 LM"),
...
save_svg("Fig_PBLM_Compartments_HiC", p)
```


```{r}
pats <- c("P01_PB", "P03_PB", "P04_PB", "P01_LM", "P03_LM", "P04_LM")
...
q <- ggplot(df, aes(mid, group = variable)) +
  geom_rect(
    data = data.frame(x = c(64e6, 92e6), y = c(68e6, 96e6)),
...
save_svg("Fig_PBLM_Compartments_CS", q)
```
...
df <- data.table(
  x = c(df$P01_PB, df$P01_PB, df$P03_PB, df$P04_PB),
  y = c(df$P03_PB, df$P01_LM, df$P03_LM, df$P04_LM),
...
save_svg("Fig_PBLM_compartment_correlations", p)
```
...
df <- data.table(
  x = c(df$P01_PB, df$P01_PB, df$P03_PB, df$P04_PB),
  y = c(df$P03_PB, df$P01_LM, df$P03_LM, df$P04_LM),
  group = rep(groups, each = nrow(df))
)
df <- df[!is.na(df$x) & !is.na(df$y)]
df$group <- factor(df$group, groups)

df[, cor(x, y)^2, by = "group"]
